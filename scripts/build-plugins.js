...
const plugins = fs
  .readdirSync(path.join(__dirname, "../node_modules/svgo/plugins/"))
  .filter(p => p[0] !== "_")
  .map(p => p.replace(".js", ""));

// generate the file which will bundle the plugins
fs.writeFileSync(
  path.join(__dirname, "../src/svgo-plugins.js"),
...
plugins.forEach(p => {
  ...
  if (index === -1) {
    console.log(
      `⚠️   Missing documentation for ${p} (./node_modules/svgo/plugins/${p}.js)`
    );
  } else {
